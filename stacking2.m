classdef stacking2 < handle
    ...
    methods
        function obj = stacking2(learner)
        ...
        function fit(obj,x,y,times)
            ...
            for i = 1:times
                if i == 1
                    ...
                    grad = -(mean(((pred_y.*obj.y)<1).*obj.y.*obj.x,1) )';
                else
                    grad = grad_next;
                end
                ...
                stepsize_f = @(a) loss((obj.x * (obj.beta + a*delta_x)),obj.y);
                stepsize = PSO(stepsize_f);
                obj.beta = obj.beta + stepsize*delta_x;

                pred_y = obj.x * obj.beta;
                grad_next = -(mean(((pred_y.*obj.y)<1).*obj.y.*obj.x,1) )'; %- 1e-4 *obj.beta'
                delta_grad = grad_next - grad;
                H = energy_Hessian(delta_grad,delta_x,H);
                disp(loss(pred_y,obj.y))
            end
        end
